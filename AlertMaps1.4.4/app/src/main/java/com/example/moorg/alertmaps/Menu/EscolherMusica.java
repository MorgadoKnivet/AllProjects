    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent){
        if(requestCode == IMAGEM_INTERNA){
            if (resultCode == RESULT_OK){
                ...
                String[] colunas = {MediaStore.Audio.Media.DISPLAY_NAME};
                Cursor cursor = getContentResolver().query(music,colunas,null,null,null);
                cursor.moveToFirst();
                ...
                /*// SDCARD
               						  	    File sdCard = Environment.getExternalStorageDirectory();ile sdcard = Environment.getExternalStorageDirectory();
               						     	File file = new File(sdCard, "Music/Music_01.mp3"); File file = new File(sdcard,)
